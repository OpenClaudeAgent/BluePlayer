 ...
 * 
 * Inspiré par TwitchAdSolutions vaft (https://github.com/pixeltris/TwitchAdSolutions)
 * 
 ...
 * 3. Si pubs détectées -> DEMANDE UN NOUVEAU TOKEN (pas juste retry le même playlist)
...
public:
   ...
   */
  void startFiltering(const QString& masterPlaylistUrl, const QString& preferredQuality = "chunked");
...
private:
  ...
  QString selectBestVariant(const QList<StreamVariant>& variants, const QString& preferredQuality);
  ...
  int m_retryCount = 0;
  static constexpr int MAX_RETRIES = 2;  // Réduit pour être plus rapide
