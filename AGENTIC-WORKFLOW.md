...
| Executeur | Implementation | Code, fonctionnalites | Plans |
| Quality | Strategie QA | Plans de test manuels, scenarios | Plans, code |
...
**Exception** : Le tandem Tester-Refactoring peut collaborer directement, mais uniquement avec validation explicite de l'utilisateur avant chaque invocation.
...
- Implementer les specifications techniques
...
|----------|-------------|
| Code source | Implementation dans `src/` |
| Tests | Tests associes dans `tests/` |
...
```
[Selection] --> [Preparation] --> [Implementation] --> [Validation] --> [Finalisation]
     |               |                  |                   |                |
     v               v                  v                   v                v
  Prochaine     Branche Git         Code +             Checklist        Commit +
  tache         + sync main         Build OK           utilisateur      Proposition merge
```
...
3. **Implementation** : Developpement selon les specifications du plan
4. **Validation** : Presentation de la checklist, iterations jusqu'a validation complete
5. **Finalisation** : Commit, mise a jour des statuts, proposition de merge
